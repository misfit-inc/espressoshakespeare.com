/**
 * SlideDeck Pro for WordPress 1.4 - 2011-03-30
...
(function($){
	...
	$(document).ready(function(){
		...
		function setNavigation(slug){
			if( slug in oc( ['image_caption_bottom','image_caption_top','image_no_caption','vertical-dark','vertical-light','vertical-stacked-arrow','vertical-stacked'] ) ){
		...
		if(document.getElementById('slide-skin')){
            var skinSelect = $('#slide-skin');
			var skinOptions = {};
				skinOptions.activeCorners = $('input[name="slidedeck_options[activeCorner]"]');
				skinOptions.hideSpines = $('input[name="slidedeck_options[hideSpines]"]');
				skinOptions.newVertical = $('input[name="slidedeck_options[useNewVertical]"]');
			
			function disableSkinOptions(){
				// uncheck and disable activeCorners option
				skinOptions.activeCorners[0].checked = false;
				skinOptions.activeCorners[0].disabled = true;
				skinOptions.activeCorners.closest('label').append('<input type="hidden" name="slidedeck_options[activeCorner]" value="false" />');
				skinOptions.activeCorners.closest('label').addClass('disabled');
				
				// uncheck and disable hideSpines option
				skinOptions.hideSpines[0].checked = true;
				skinOptions.hideSpines[0].disabled = true;
				skinOptions.hideSpines.closest('label').append('<input type="hidden" name="slidedeck_options[hideSpines]" value="true" />');
				skinOptions.hideSpines.closest('label').addClass('disabled');
			}
			
			if( skinSelect.val() == 'stacked-nav' || skinSelect.val() == 'stacked-nav-arrow' ){
				disableSkinOptions();
			}
			skinSelect.change(function(event){
                if( skinSelect.val() == 'stacked-nav' || skinSelect.val() == 'stacked-nav-arrow'){
                    disableSkinOptions();
                }else{
					// remove disable on activeCorners option
					skinOptions.activeCorners[0].disabled = false;
					skinOptions.activeCorners.closest('input[type="hidden"]').remove();
					skinOptions.activeCorners.closest('label').removeClass('disabled');
					
					// remove disable on hideSpines option
					skinOptions.hideSpines[0].checked = false;
					skinOptions.hideSpines[0].disabled = false;
					skinOptions.hideSpines.closest('input[type="hidden"]').remove();
					skinOptions.hideSpines.closest('label').removeClass('disabled');
				}
				
				if( skinSelect.val() in oc( ['darknav','default','invasion','literally','ribbons','slidedeck-classic','stacked-nav','stacked-nav-arrow','stitch','voyager','noisy'] ) ){
					skinOptions.newVertical[0].checked = true;
				}
            });
        }
