...
function slidedeck_dynamic() {
    ...
    if( function_exists('get_post_types') ){
        $publicly_queryable = get_post_types( array('publicly_queryable' => true), 'object' );
        if( !empty( $publicly_queryable ) ){
            $is_publicly_queryable = true;
        }
    }
    
    if( function_exists('get_post_types') && $is_publicly_queryable ){
        $post_types_raw = get_post_types( array(
            'publicly_queryable' => true    // Filter down to only those post types that are publicly publicly queryable
        ), 'objects' );
                        ...
                        switch ( $key2 ) {
                            ...
                            case "vertical_content":
                                ...
                                foreach( $val2 as $v_slide ) {
                                    $sanitized[] = slidedeck_sanitize( $v_slide );
                                }
                                $sanitized = $sanitized;
                            break;
                        }
...
function slidedeck_process_template( $slidedeck_id, $styles = array( 'width' => '100%', 'height' => '300px' ) ) {
    ...
    if ( isset( $slidedeck ) && !empty( $slidedeck ) ) {
        ...
        $skin_image_width = isset( $skin['meta']['Skin Image Width'] ) ? $skin['meta']['Skin Image Width'] : '300px';
        ...
        $slidedeck_footer_scripts .= '<script type="text/javascript">jQuery( \'#' . $slidedeck_uid . '\' ).slidedeck( ' . $slidedeck_options_json . ' )' . $vertical_addition . ';</script><noscript>Powered by <a href="http://www.slidedeck.com" target="_blank">SlideDeck WordPress Slider</a></noscript>';
	...
	if( !$vertical_smart_skin ){
		foreach ( (array) $slides as $slide ) {
	        ...
	        if( $is_vertical ) {
	            ...
	            $template_str.= slidedeck_process_slide_content( $slide['content'] );        // Slide Content
	        }
...
function _d( $var, $verbose = false ) {
	return slidedeck_debug( $var, $verbose );
}
