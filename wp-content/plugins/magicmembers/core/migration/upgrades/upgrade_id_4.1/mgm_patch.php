...
function mgm_patch_get_users($start, $limit) {
	...
	foreach ($uids as $user_id) {	
		...
		$pack = mgm_get_option('subscription_packs')->get_pack($mgm_member->pack_id);
		if(!empty($pack['membership_type']) && $pack['membership_type'] != $mgm_member->membership_type && $mgm_member->status == MGM_STATUS_ACTIVE) {
			$mgm_member->membership_type = $pack['membership_type']; 
