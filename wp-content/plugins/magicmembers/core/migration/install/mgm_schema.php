<?php 
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_COUNTRY . '` (
		...
		PRIMARY KEY (`id`)
	) ENGINE=MyISAM COMMENT = "countries"';
 $wpdb->query($sql); 
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_DOWNLOAD . '` (
			...
			`expire_dt` datetime NULL,
			PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "downloads"';
 $wpdb->query($sql); 
 
 
 // download attributes : mgm_download_attributes
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_DOWNLOAD_ATTRIBUTE . '` (
			...
			PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "download attributes"';
 $wpdb->query($sql);
 
 // download attribute types : mgm_download_attribute_types
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_DOWNLOAD_ATTRIBUTE_TYPE . '` (
			...
			PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "download attribute types"';
 $wpdb->query($sql);
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_DOWNLOAD_POST_ASSOC . '` (
		...
		) ENGINE=MyISAM COMMENT = "download post association"';
 $wpdb->query($sql); 
 
 // posts purchased : mgm_posts_purchased
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_POSTS_PURCHASED . '` (
			`id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
			`user_id` bigint(20) unsigned NOT NULL,
			`post_id` bigint(20) unsigned NOT NULL,
			...
			PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "post purchase/gift log"';
 $wpdb->query($sql); 	
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_POST_PACK . '` (
			...
			PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "post packs"';
 $wpdb->query($sql);
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_POST_PACK_POST_ASSOC . '` (
			...
			`create_dt` datetime NULL,
			PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "post pack associations"';
 $wpdb->query($sql);	 
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_TRANSACTION . '` (
		...
		) ENGINE=MyISAM COMMENT = "transaction data log"';
 $wpdb->query($sql);
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_POST_PROTECTED_URL . '` (
		...
		PRIMARY KEY (`id`)
	) ENGINE=MyISAM COMMENT = "protected post url"';
 $wpdb->query($sql);  
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_REST_API_KEY . '` (
		...
		`create_dt` DATETIME NOT NULL,
		PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "rest api keys"';
 $wpdb->query($sql);
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_REST_API_LEVEL . '` (
		...
		PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "rest api access levels"';
 $wpdb->query($sql);
 ...
 $sql = 'CREATE TABLE IF NOT EXISTS `' . TBL_MGM_REST_API_LOG . '` (
		...
		`create_dt` DATETIME NOT NULL,
		PRIMARY KEY (`id`)
		) ENGINE=MyISAM COMMENT = "rest api logs"';
 $wpdb->query($sql);
 ...
 $sql = "INSERT IGNORE INTO `" . TBL_MGM_COUNTRY. "` (`id`, `name`, `code`) VALUES 
  ...
  // ---------------------------------------------- api default data ---------------------------------------------------------
  // level
  $sql = "INSERT IGNORE INTO `" . TBL_MGM_REST_API_LEVEL. "` (`id`, `level_name`, `permissions`, `limits`) VALUES 
		 (1, 'full access', '[]', 1000);";
  $wpdb->query($sql);
  // keys
