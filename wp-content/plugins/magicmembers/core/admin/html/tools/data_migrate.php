...
<?php mgm_box_top('Migrate Old Magic Members Data');?>
	<form name="frmdatamgr" id="frmdatamgr" action="admin.php?page=mgm/admin/tools&method=data_migrate" method="post">
		<table width="100%" cellpadding="1" cellspacing="0" border="0" class="form-table">				
			<tbody>
				<tr>
					<td><b><?php _e('Please Select data options to migrate :','mgm')?></b></td>
				</tr>
				<tr>
					<td>
						<?php 
				...
				<tr>
					<td style="border-top:1px solid #D7E5EE; height:25px; ">
						...
						<p>
							<div id="import_file_box" style="display:none">
				...
				<tr>
					<td height="10px">
						<p>					
							...
							<?php endif;?>
							<input type="button" class="button" onclick="data_migrate()" value="<?php _e('MIGRATE &raquo;','mgm') ?>" />
						</p>
					</td>
				</tr>	
			</tfoot>
...
<form name="frmimportusers" id="frmimportusers" action="admin.php?page=mgm/admin/tools&method=data_migrate" method="post">
<table width="100%" cellpadding="1" cellspacing="0" border="0" class="form-table">				
<tbody>
	<tr>
		<td width="30%" valign="top" valign="left"><b><?php _e('Please Select File:','mgm')?></b></td>
	...
	<div class="information"><?php _e('Supported File Types: <b>CSV/XLS</b> <br/>Minimum Fileds required: [<b>user_login, user_email, membership_type, pack_id</b>]. <br/><b>Custom field Names</b> can also be included.<br/>Maximum number of records that can be imported at once: <b>' . $data['import_limit'].'</b>','mgm')?></div>	
	</td>
	</tr>
</tbody>
<tfoot>	
	<tr>
		<td height="10px" colspan="2">
			<p>				
			<input type="button" class="button" onclick="import_users_submit()" value="<?php _e('Import Users&raquo;','mgm') ?>" />
			</p>
		</td>
	</tr>	
</tfoot>
...
<!--
	jQuery(document).ready(function(){		
		...
		mgm_upload_import_file=function(obj) {
			...
			if(jQuery(obj).val().toString().is_empty()==false){	
				...
				jQuery.ajaxFileUpload({
						url:'admin.php?page=mgm/admin/tools&method=import_file_upload', 
						secureuri:false,
		...
		check_import_status = function(data, retry_count) {
			jQuery.ajax({url: 'admin.php?page=mgm/admin/tools&method=data_migrate', type: 'POST',dataType: 'json', data: {migrate_execute: 'true', migrate_type: 'import_status', import_users: jQuery('#frmimportusers input[name="import_users"]').val(), retry : retry_count },
		...
		mgm_upload_importusers_file=function(obj) {
			...
			if(jQuery(obj).val().toString().is_empty()==false){	
				...
				// upload 
				jQuery.ajaxFileUpload({
						url:'admin.php?page=mgm/admin/tools&method=importusers_file_upload', 
						secureuri:false,
						fileElementId:jQuery(obj).attr('id'),
						dataType: 'json',						
						success: function (data, status){	
