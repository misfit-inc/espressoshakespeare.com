...
class mgm_xls_writer{
	...
	function xls_write_value($row, $col, $value) {
		$this->xls_string.= pack("sssss", 0x203, 14, $row, $col, 0x0);
		$this->xls_string.= pack("d", $value);
		return;
	}
	...
	function xls_write_label($row, $col, $label ) {
		$length = strlen($label);
		$this->xls_string.= pack("ssssss", 0x204, 8 + $length, $row, $col, 0x0, $length);
		$this->xls_string.= $label;
		return;
	} 	
